...
BasicAnimationWindow::BasicAnimationWindow(QWidget *parent) : QWidget(parent),
...
void BasicAnimationWindow::init()
{
    ...
    m_vtkWidget->renderWindow()->GetInteractor()->AddObserver(vtkCommand::TimerEvent, m_animCallback);
    this->setWindowTitle("BasicAnimationWindow");
}

void BasicAnimationWindow::startTimer()
{
    m_timerId = m_vtkWidget->renderWindow()->GetInteractor()->CreateRepeatingTimer(m_interval);
}

void BasicAnimationWindow::stopTimer()
{
    m_vtkWidget->renderWindow()->GetInteractor()->DestroyTimer(m_timerId);
}
