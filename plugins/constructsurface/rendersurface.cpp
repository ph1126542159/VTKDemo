...
RenderSurface::RenderSurface(QWidget *parent) : QWidget(parent),
...
void RenderSurface::init()
{
    ...
    QStringList fileList = QStringList() << "ackley"
                                         << "data"
                                         << "demo"
                                         << "griewank"
                                         << "pit"
...
void RenderSurface::onPointsToPolyDataConverterFinished()
{
    ...
    if (m_scalarBar)
    {
        VTK_CREATE(vtkLookupTable, lutt);
        lutt->SetHueRange(0.6667, 0);
        lutt->SetRange(m_scalarMin, m_scalarMax);
        lutt->SetNumberOfColors(m_numOfScalarColors);
        lutt->Build();
        m_scalarBar->SetLookupTable(lutt);
    }
...
void RenderSurface::showContourLabels(bool show)
{
    ...
    while (cells->GetNextCell(numberOfPoints, const_cast<const vtkIdType* &>(indices)))
    {
        lineCount++;
...
void RenderSurface::showBandedContour(bool )
{
    // TODO
}
