...
void ContourWindow::on_plotScalarsCheckBox_toggled(bool checked)
{
    ...
    if (checked) {
        if (!m_lineWidget) {
            ...
            m_lineWidget->SetInteractor(m_vtkWidget->interactor());
...
void ContourWindow::onMousePressed(double* )
{
    if (!m_isPickPoint)
        return;
}
...
void ContourWindow::onLineWidgetPointsChanged(double* pot1, double* pot2)
{
    ...
    if (!m_contextView) {
        ...
        m_contextView->SetRenderWindow(m_plotWidget->renderWindow());
        ...
        m_linePlot->SetColorF(clr.GetRed(), clr.GetGreen(), clr.GetBlue());
