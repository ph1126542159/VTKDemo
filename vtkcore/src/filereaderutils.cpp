...
#define MAX_ANSYS_LINE 200
        ...
        for (int lineIndex = 0; !ifile.eof(); ++lineIndex)
        {
...
#if defined(_WIN32)
            std::size_t linelen = strlen(line);
#else
            std::size_t linelen = strlen(line) - 1; // account for the end of line char.
#endif
...
#if 0
                debug5 << "Padding line with NULLs" << Qt::endl;
                debug5 << line << Qt::endl;
#endif
            }
            ...
            else if (STRNCASECMP(line, "NBLOCK", 6) == 0)
            {
                ...
                {
                    qDebug() << "Malformed format string: " << line << Qt::endl;
                    fatalError = true;
                }
            }
        ...
        int& linelen) const
    {
        ...
        qDebug() << "avtANSYSFileFormat::Interpret: " << fmt << Qt::endl;

        // Example: 6e16.9
        if (sscanf_s(fmt, "%de%d.%d", &i0, &i1, &i2) == 3)
        {
            linelen = i0 * i1 / 2;
            fieldWidth = i1;
        }
        // Example: 19i7
        else if (sscanf_s(fmt, "%di%d", &i0, &i1) == 2)
        {
            linelen = i0 * i1;
            fieldWidth = i1;
        }
        ...
        {
            qDebug() << "Invalid format string: " << fmt << Qt::endl;
        }
    }
